...
# find.descendants just returns the vector of the
# the descendants for a given row, while find.all.descendants
# returns a list with all the descendants for each row, which
# might be more efficient if we need to do it more than
# once. 
#########################################################
...
find.all.descendants <- function(nodes){
  nr<-NROW(nodes)
  all.list<-vector("list",nr)
  ...
  for(i in 1:nr){
    if(nodes[i,3]==0){
      ...
      all.list[[i]]<-c(all.list[[nodes[i,3]]],all.list[[nodes[i,4]]])
    }
  }
  return(all.list)
}
